...
def alpha_beta(state, depth, alpha, beta, visited, previous_action=None):
    ...
    for action in actions:
        ...
        if key in visited:
            continue
        visited.add(key)
        value = alpha_beta(next_state, depth - 1, alpha, beta, visited, action)
        visited.remove(key)
        best_value = max(best_value, value)
        alpha = max(alpha, best_value)
        if beta <= alpha:
            log_prune("Tia nhanh alpha-beta")
            break
...
def solve_with_alpha_beta(state, depth=8, max_steps=30):
...
if __name__ == "__main__":
    print("ALPHA-BETA - 8 PUZZLE")
    start = make_initial_state(["UP", "LEFT", "LEFT", "DOWN", "RIGHT", "UP"])
    print("Trang thai ban dau:")
    print_table(start)

    solution, final_state = solve_with_alpha_beta(start, depth=8, max_steps=30)
