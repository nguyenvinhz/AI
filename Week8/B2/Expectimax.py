...
def expectimax(state, depth, maximizing_player, visited, previous_action=None):
    ...
    if maximizing_player:
        ...
        for action in actions:
            ...
            if key in visited:
                continue
            visited.add(key)
            value = expectimax(next_state, depth - 1, False, visited, action)
            visited.remove(key)
    ...
    for action in actions:
        ...
        if key in visited:
            continue
        visited.add(key)
        total += expectimax(next_state, depth - 1, True, visited, action)
        visited.remove(key)
        count += 1

    return total / count if count > 0 else evaluate(state)
...
def choose_move(state, depth, previous_action=None):
    ...
    for action in ordered_actions(state, previous_action):
        ...
        value = expectimax(next_state, depth - 1, False, visited, action)
        visited.remove(key)
        log(f"Thu {action}: expected value={value}, h(n)={manhattan_distance(next_state)}")
        if value > best_value:
            best_value = value
            best_action = action
...
def solve_with_expectimax(state, depth=6, max_steps=30):
    ...
    for step in range(max_steps):
        ...
        key = state_to_string(next_state)
        log(f"Chon {action}: expected value={value}")
        if key in visited:
            return None, current
...
if __name__ == "__main__":
    print("EXPECTIMAX - 8 PUZZLE")
    start = make_initial_state(["UP", "LEFT", "LEFT", "DOWN", "RIGHT", "UP"])
    print("Trang thai ban dau:")
    print_table(start)

    solution, final_state = solve_with_expectimax(start, depth=6, max_steps=30)
